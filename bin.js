...
var about = {
	main: function() {
		...
		render(
			...
			"Run 'settings all' in the terminal to see all configureable items.\n"+
	...
	main: function(args) {
		switch(args[0]?.toString().toLowerCase()) {
			case undefined: {
				...
				render("   Style\n   Terminal", 'dodgerblue');
				render("\n\nTo see all of the settings at once, use 'settings all'.")
				break;
			} case 'style': {
				render('Style settings (Font / Background)', 'dodgerblue');
				render('   Current Font: ' + window.getComputedStyle(document.getElementById('terminal-container')).fontFamily);
				render('   Current Font Size: ' + window.getComputedStyle(document.getElementById('terminal-container')).fontSize);
				break;
			} case 'terminal': {
				render('Terminal Settings (Caret / REPL)', 'dodgerblue')
				render("   Current REPL: "+getREPL(1));
				break;
			} case 'all': {
				switch(args[1]?.toString().toLowerCase()) {
					case undefined: {
						render('All Settings', "dodgerblue")
						render('   Current Font: ' + window.getComputedStyle(document.getElementById('terminal-container')).fontFamily);
						render('   Current Font Size: ' + window.getComputedStyle(document.getElementById('terminal-container')).fontSize);
						render("   Current REPL: " + getREPL(1));
						render(`   Bangs Enabled (!bang): ${terminal.bangs ? 'Yes' : 'No'}`);
					} case '--configure': {
						render('All Settings', "dodgerblue")
						render('   fontFamily ' + window.getComputedStyle(document.getElementById('terminal-container')).fontFamily);
						render('   font-sizze' + window.getComputedStyle(document.getElementById('terminal-container')).fontSize);
						render("   Current REPL: " + getREPL(1));
						render(`   Bangs Enabled (!bang): ${terminal.bangs ? 'Yes' : 'No'}`);
					}
				}
