...
function updatePrompt() {
	$("#prompt").text(getName()+"@"+getMachine()+getREPL());
...
function getREPL(args) {
    if (args == 1) {
        return terminal.userREPL.replaceAll('\n', '\\n')
    } else return terminal.userREPL || ":$";
}
...
function appendLastInput(text) {
	var inputPre = '<p><span class="prompt">'+getName()+'@'+getMachine()+getREPL()+'&nbsp;</span><span class="input-old">';
	var inputSuf = '</span></p>';
...
function getBrowser(arg) {
    ...
    if (arg == 1) {
        return browserName
    } else return browserName+' '+fullVersion
}
